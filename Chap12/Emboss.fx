...
<
   string ResourceName = "D:\\source\\05_DiffuseSpecularMapping\\fieldstone_SM.tga";
>;
...
sampler2D ColorConversion_Emboss_Pixel_Shader_SceneSampler = sampler_state
{
   Texture = (SceneTexture_Tex);
};
...
float4 ColorConversion_Emboss_Pixel_Shader_ps_main(ColorConversion_Emboss_Pixel_Shader_PS_INPUT Input) : COLOR
{
    ...
    for (int y = -1; y <= 1; ++y)
    {
        for (int x = -1; x <= 1; ++x)
        {
            ...
            float3 tex = tex2D(ColorConversion_Emboss_Pixel_Shader_SceneSampler, Input.mUV + offset).rgb;
            float luminance = dot(tex, float3(0.3, 0.59, 0.11));
            
            res += luminance * Kx[y + 1][x + 1];
        }
    }
