...
sampler2D ColorConversion_EdgeDetection_Pixel_Shader_SceneSampler = sampler_state
{
   Texture = (SceneTexture_Tex);
};
...
float4 ColorConversion_EdgeDetection_Pixel_Shader_ps_main(ColorConversion_EdgeDetection_Pixel_Shader_PS_INPUT Input) : COLOR
{
    ...
    for (int y = -1; y <= 1; ++y)
    {
        for (int x = -1; x <= 1; ++x)
        {
            ...
            float3 tex = tex2D(ColorConversion_EdgeDetection_Pixel_Shader_SceneSampler, Input.mUV + offset).rgb;
